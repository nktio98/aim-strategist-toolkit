...
# ------------------------------------------------------ downgrade mechanism
def add_future_downgrades(panel: pd.DataFrame,
                          horizons: tuple[int, ...] = (1, 3, 6, 12)
                          ) -> pd.DataFrame:
    """dg_h = 1 if the bond's RATING_NUM worsens (increases) between t
    and t+h, requiring h consecutive calendar months of coverage."""
    p = panel.sort_values(["ISSUE_ID", "DATE"]).copy()
    p["RATING_NUM"] = pd.to_numeric(p.get("RATING_NUM"), errors="coerce")
    g = p.groupby("ISSUE_ID")
    mnum = p["DATE"].dt.year * 12 + p["DATE"].dt.month
    for h in horizons:
        fut = g["RATING_NUM"].shift(-h)
        gap_ok = (mnum.groupby(p["ISSUE_ID"]).shift(-h) - mnum) == h
        dg = (fut > p["RATING_NUM"]).astype(float)
        p[f"dg_{h}"] = dg.where(gap_ok & fut.notna()
                                & p["RATING_NUM"].notna())
    return p


def downgrade_mechanism(panel: pd.DataFrame,
                        horizons: tuple[int, ...] = (1, 3, 6),
                        mis_col: str = "spread_resid_w",
                        segment: str = IG_LABEL) -> dict:
    """Is 'cheap' just 'about to be downgraded'?

    (a) information test: FMB linear-probability regressions of future
        downgrades on the residual + controls. A positive significant
        slope = the residual contains credit-deterioration information.
    (b) contamination test: the return FMB excluding bond-months that
        get downgraded next month. If the premium survives, it is not
        mechanically generated by downgrade events.
    """
    p = add_future_downgrades(panel, horizons)
    seg = p[p["RATING_CLASS"] == segment]

    info_rows = []
    for h in horizons:
        out = fama_macbeth(seg, y_col=f"dg_{h}", key_cols=[mis_col])
        s = out["summary"]
        if mis_col in s.index:
            base = seg[f"dg_{h}"].mean()
            info_rows.append({
                "horizon_m": h,
                "base_downgrade_rate": float(base),
                "coef": float(s.loc[mis_col, "coef"]),
                "t_stat": float(s.loc[mis_col, "t_stat"]),
                "n_months": int(s.loc[mis_col, "n_months"])})
    info = pd.DataFrame(info_rows).set_index("horizon_m")

    base_out = fama_macbeth(seg[seg["dg_1"].notna()], key_cols=[mis_col])
    excl_out = fama_macbeth(seg[seg["dg_1"] == 0], key_cols=[mis_col])
    rows = {}
    for name, out in (("baseline (dg sample)", base_out),
                      ("excl. downgraded next month", excl_out)):
        s = out["summary"]
        if mis_col in s.index:
            rows[name] = {"coef": float(s.loc[mis_col, "coef"]),
                          "t_stat": float(s.loc[mis_col, "t_stat"]),
                          "n_months": int(s.loc[mis_col, "n_months"])}
    decomp = pd.DataFrame(rows).T
    share_dg = float((seg["dg_1"] == 1).mean())
    return {"info": info, "return_decomp": decomp,
            "share_downgraded_next_month": share_dg}
